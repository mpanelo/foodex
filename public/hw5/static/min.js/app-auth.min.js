...
const btnLogout=document.getElementById('btnLogout');if(btnLogout){btnLogout.addEventListener('click',e=>{console.log("logout clicked, logging out");firebase.auth().signOut();window.location='login.html'})}};firebase.auth().onAuthStateChanged(firebaseUser=>{console.log('auth state changed');if(firebaseUser){if(window.location.pathname=='/hw4/templates/login.html'){console.log("logging in as",firebaseUser.uid);window.location='main.html'}
else if(window.location.pathname=='/hw4/templates/signUp.html'){console.log("logging in as",firebaseUser.uid);window.location='main.html'}}else{if(window.location.pathname!='/hw4/templates/login.html'&&window.location.pathname!='/hw4/templates/signUp.html'){console.log("not logged in");window.location='login.html'}}})